...
export function MobileList({
  ...
  onMonthChange,
  onYearChange,
}: MobileListProps): React.JSX.Element {
  ...
  return (
    <Box sx={{ padding: 2 }}>
      <Box sx={boxContainerStyle}>
        ...
        <Button variant="text" onClick={handleOpenModal} sx={buttonStyle}>
      ...
      {rows.map((row, index) => (
        <Box sx={cardStyle} onClick={() => handleEditClick(row)} key={row.id}>
          <DotOutline
            size={36}
          ...
          <Box sx={{ flexGrow: 1 }}>
            <Typography variant="subtitle1">
              ...
              {row.isParcela && `(${row.numeroParcela}/${row.totalParcelas})`}
            </Typography>
